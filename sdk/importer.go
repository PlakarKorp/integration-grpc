...
func ExecImporter(ctx context.Context, proto string, params map[string]string, opts *connectors.Options, exe string, args []string) (importer.Importer, error) {
	client, err := spawn(ctx, exe, args)
	if err != nil {
		return nil, err
	}

	imp, err := grpc_importer.NewImporter(ctx, client, opts, proto, params)
	if err != nil {
		client.Close()
		return nil, err
	}
